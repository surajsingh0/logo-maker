...
const getPolygonPoints = (
    ...
    if (type === "polygon") {
        const s = element.sides || 3;
        if (radius <= 0) return "";
        for (let i = 0; i < s; i++) {
            const a = (i / s) * 2 * Math.PI - Math.PI / 2;
            points.push({ x: radius * Math.cos(a), y: radius * Math.sin(a) });
        }
    } else if (type === "star") {
        const nP = element.numPoints || 5;
        if (outerRadius <= 0) return "";
        for (let i = 0; i < nP * 2; i++) {
            const r = i % 2 === 0 ? outerRadius : innerRadius;
            const a = (i / (nP * 2)) * 2 * Math.PI - Math.PI / 2;
        ...
        if (radius <= 0) return "";
        for (let i = 0; i < 16; i++) {
            const cR = i % 2 === 0 ? radius : radius * 0.4;
            const a = (i * Math.PI) / 8;
            points.push({ x: cR * Math.cos(a), y: cR * Math.sin(a) });
        }
    }
...
const SVGElement: React.FC<SVGElementProps> = ({
    ...
    const { type, position, styles, rotation = 0 } = element;
        ...
        if (element.selected && elementRef.current) {
            try {
                ...
                if (type === "blockArrow") {
                    ...
                    setLocalBbox(
                        new DOMRect(-width / 2, -height / 2, width, height)
                    );
                } else if (type === "text") {
                ...
                {points.map((p, index) => {
                    ...
                    return (
                        <circle
                            ...
                            onMouseDown={(e) =>
                                onEndpointDown(element.id, index, e)
                            }
                        />
                    );
                })}
    ...
    const renderElementContent = () => {
        switch (type) {
            ...
            case "cloud": {
                ...
                const transformedPathData = pathData.replace(
                    /([0-9.-]+),([0-9.-]+)/g,
                    (_, x, y) =>
                        `${parseFloat(x) - width / 2},${
                            parseFloat(y) - height / 2
                        }`
                );
                return (
                    <g ref={elementRef as any} transform={centerGroupTransform}>
                    ...
                    <g ref={elementRef as any} transform={centerGroupTransform}>
                        {(() => {
                            if (type === "circle") {
                                ...
                                // Polygons/Stars
                                const pointsStr = getPolygonPoints(
                                    type,
                                    element
                                );
    ...
    return (
        <>
            ...
            {element.selected && element.locked && (
                <g transform={centerGroupTransform}>
                    {" "}
                    {/* Assuming centered lock icon is fine */}
                    <text
                        x={relativeCoords.x}
                        y={relativeCoords.y - 10}
                        fontSize="10"
                        textAnchor="middle"
                        fill="red"
                    >
