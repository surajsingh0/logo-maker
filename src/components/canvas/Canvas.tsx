...
interface CanvasProps {
    ...
    onElementDrag: (
        elementId: string,
        updates: Partial<Pick<LogoElement, "position" | "points">>
    ) => void;
...
const Canvas: React.FC<CanvasProps> = ({
    ...
    const [resizeStartState, setResizeStartState] =
        useState<ResizeStartState | null>(null);
    const [isMovingEndpoint, setIsMovingEndpoint] = useState<boolean>(false);
    const [movingPointInfo, setMovingPointInfo] =
        useState<MovingPointInfo | null>(null);
    const [isMarqueeSelecting, setIsMarqueeSelecting] =
        useState<boolean>(false);
    const [marqueeStartPos, setMarqueeStartPos] = useState<Position | null>(
        null
    );
    const [marqueeEndPos, setMarqueeEndPos] = useState<Position | null>(null);
    const [isDraggingMultiple, setIsDraggingMultiple] =
        useState<boolean>(false);
    const [controlsStyle, setControlsStyle] = useState({
        bottom: "16px",
        right: "16px",
    });
    const [showVerticalGuide, setShowVerticalGuide] = useState<number | null>(
        null
    );
    const [showHorizontalGuide, setShowHorizontalGuide] = useState<
        number | null
    >(null);
    ...
    const handleResizeStart = (handle: string, event: React.MouseEvent) => {
        ...
        if (
            !element ||
            element.locked ||
            ["line", "arrow", "curvedLine"].includes(element.type)
        )
            return;
    ...
    const getCenterFromBounds = (
        bounds: NonNullable<ElementBounds>
    ): Position => {
        return {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        const calculateAndSetGuides = (
            activeId: string,
            potentialBounds: ElementBounds
        ) => {
            if (!potentialBounds) {
                return;
            }
            ...
            if (
                Math.abs(activeCenter.x - targetX) <
                CENTERING_TOLERANCE / zoomLevel
            ) {
                currentVerticalGuide = targetX;
            }
            // Check for horizontal alignment
            if (
                Math.abs(activeCenter.y - targetY) <
                CENTERING_TOLERANCE / zoomLevel
            ) {
        ...
        if (isMovingEndpoint && movingPointInfo) {
            ...
            switch (initialElementState.type) {
                ...
                case "star": {
                    const newOuterRadius = Math.max(
                        5,
                        Math.max(Math.abs(dx), Math.abs(dy))
                    );
                    let newInnerRadius = initialElementState.innerRadius || 0;
                    const initialOuterRadius =
                        initialElementState.outerRadius || 0;
                ...
                case "text": {
                    ...
                    const { fontSize: initialFontSize = 16 } =
                        initialElementState;
                    const delta =
                        Math.abs(dx_delta) > Math.abs(dy_delta)
                            ? dx_delta
                            : dy_delta;
                    let scaleFactor = 0;
                    if (
                    ...
                    let newFontSize =
                        initialFontSize + scaleFactor * sensitivity;
                ...
                case "cloud": {
                    ...
                    if (resizeHandle.includes("e")) {
                        newWidth = Math.max(
                            10,
                            initialDimensions.width + dx_delta
                        );
                        ...
                        newX =
                            initialPosition.x +
                            (initialDimensions.width - calculatedWidth);
                        newWidth = calculatedWidth;
                    }
                    if (resizeHandle.includes("s")) {
                        newHeight = Math.max(
                            10,
                            initialDimensions.height + dy_delta
                        );
                        ...
                        newY =
                            initialPosition.y +
                            (initialDimensions.height - calculatedHeight);
                    ...
                    const scaleX =
                        initialDimensions.width === 0
                            ? 1
                            : newWidth / initialDimensions.width;
                    const scaleY =
                        initialDimensions.height === 0
                            ? 1
                            : newHeight / initialDimensions.height;

                    const pathCommands =
                        initialPathData.match(/[A-Z][^A-Za-z]*/g) || [];
                    const scaledCommands = pathCommands.map((cmd) => {
                        ...
                        switch (command) {
                            case "M":
                            case "L":
                                return `${command}${coords[0] * scaleX} ${
                                    coords[1] * scaleY
                                }`;
                            case "C":
                                return `${command}${coords[0] * scaleX} ${
                ...
                case "blockArrow": {
                    ...
                    const angleRad =
                        -(initialElementState.rotation || 0) * (Math.PI / 180);
                    const cos = Math.cos(angleRad);
                    const sin = Math.sin(angleRad);

                    // Transform the mouse movement into the rotated space
                    const dx_rotated =
                        (currentMousePos.x - initialMousePos.x) * cos -
                        (currentMousePos.y - initialMousePos.y) * sin;
                    const dy_rotated =
                        (currentMousePos.x - initialMousePos.x) * sin +
                        (currentMousePos.y - initialMousePos.y) * cos;
                    ...
                    if (resizeHandle.includes("e")) {
                        newWidth = Math.max(
                            20,
                            initialDimensions.width + dx_rotated * 2
                        );
                    } else if (resizeHandle.includes("w")) {
                        newWidth = Math.max(
                            20,
                    ...
                    if (resizeHandle.includes("s")) {
                        newHeight = Math.max(
                            10,
                            initialDimensions.height + dy_rotated * 2
                        );
                    } else if (resizeHandle.includes("n")) {
                        newHeight = Math.max(
                            10,
                ...
                default:
                    console.warn(
                        "Resize logic not fully implemented for type:",
            ...
            if (Object.keys(update).length > 0) {
                ...
                if (currentElement) {
                    ...
                    const potentialBounds = getElementBounds(
                        potentialElementState
                    );
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        if (
            (e.target as SVGElement).closest(".resize-handle") ||
        ...
        const selectedElementsCount = elements.filter(
            (el) => el.selected
        ).length;
        ...
        if (
            ...
            if (shiftKeyPressed) {
                const currentSelection = elements
                    .filter((el) => el.selected)
                    .map((el) => el.id);
                let newSelectionIds;
                if (isClickOnSelectedElement) {
                    newSelectionIds = currentSelection.filter(
                        (id) => id !== clickedElementId
                    );
                } else {
                ...
                >
                    ...
                    {isMarqueeSelecting && marqueeStartPos && marqueeEndPos && (
                        <rect
                            ...
                            width={Math.abs(
                                marqueeStartPos.x - marqueeEndPos.x
                            )}
                            height={Math.abs(
                                marqueeStartPos.y - marqueeEndPos.y
                            )}
