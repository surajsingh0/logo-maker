...
export const createCurvedLine = (
    ...
    return {
        ...
        type: "curvedLine",
        position,
        points: [relativeStart, relativeControl, relativeEnd],
        styles: {
            fill: "none",
            stroke: "#3498db",
            strokeWidth: 3,
...
export const createOctagonStar = (
    position: Position,
    radius: number = 50
): LogoElement => {
    return {
...
const calculateBlockArrowPoints = (
    width: number,
    height: number
): Position[] => {
    ...
    switch (type) {
        ...
        case "line": {
            ...
            if (lenSq === 0) {
                // Points are the same
                return (
                    (rotatedX - relP1x) ** 2 + (rotatedY - relP1y) ** 2 <=
                    tolerance ** 2
                );
            }

            let t =
                ((rotatedX - relP1x) * (relP2x - relP1x) +
                    (rotatedY - relP1y) * (relP2y - relP1y)) /
                lenSq;
            ...
            const distSq =
                (rotatedX - closestX) ** 2 + (rotatedY - closestY) ** 2;
        ...
        case "arrow": {
            ...
            // Check main line
            const lenSq =
                (relEndX - relStartX) ** 2 + (relEndY - relStartY) ** 2;
            if (lenSq === 0)
                return (
                    (rotatedX - relStartX) ** 2 + (rotatedY - relStartY) ** 2 <=
                    tolerance ** 2
                );

            let t =
                ((rotatedX - relStartX) * (relEndX - relStartX) +
                    (rotatedY - relStartY) * (relEndY - relStartY)) /
                lenSq;
            ...
            const arrowHead1X =
                relEndX - arrowHeadSize * Math.cos(angle - Math.PI / 6);
            const arrowHead1Y =
                relEndY - arrowHeadSize * Math.sin(angle - Math.PI / 6);
            const arrowHead2X =
                relEndX - arrowHeadSize * Math.cos(angle + Math.PI / 6);
            const arrowHead2Y =
                relEndY - arrowHeadSize * Math.sin(angle + Math.PI / 6);
            ...
            return (
                distSq <= tolerance ** 2 ||
                distToHead1 <= tolerance ||
                distToHead2 <= tolerance
            );
        }
        ...
        case "curvedLine": {
            ...
            const distToStart =
                (rotatedX - start.x) ** 2 + (rotatedY - start.y) ** 2;
            const distToControl =
                (rotatedX - control.x) ** 2 + (rotatedY - control.y) ** 2;
            const distToEnd = (rotatedX - end.x) ** 2 + (rotatedY - end.y) ** 2;

            if (
                distToStart <= handleRadiusSq ||
                distToControl <= handleRadiusSq ||
                distToEnd <= handleRadiusSq
            ) {
                return true;
            }
            ...
            for (let i = 0; i < numSegments; i++) {
                ...
                let t =
                    ((rotatedX - p1.x) * (p2.x - p1.x) +
                        (rotatedY - p1.y) * (p2.y - p1.y)) /
                    lenSq;
                t = Math.max(0, Math.min(1, t));
                ...
                const distSq =
                    (rotatedX - projX) ** 2 + (rotatedY - projY) ** 2;
        ...
        case "path": {
            ...
            return (
                relativeLeft <= rotatedX &&
                rotatedX <= relativeRight &&
                relativeTop <= rotatedY &&
                rotatedY <= relativeBottom
            );
        }
        ...
        case "pentagon":
        case "octagonStar": {
            // Polygons, stars, hexagons, pentagons, and octagon stars use the same logic
...
export const updateElementOnResize = (
    element: LogoElement,
    handle: string,
    dx: number,
    dy: number,
    ...
    switch (element.type) {
        ...
        case "cloud": {
            ...
            const constrained = enforceMinSize(element, {
                width: newWidth,
                height: newHeight,
            });
        ...
        case "circle": {
            ...
            const newRadius =
                (element.radius || 0) +
                (handle.includes("w") || handle.includes("n") ? -delta : delta);
