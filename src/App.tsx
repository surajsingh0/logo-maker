...
function App() {
    ...
    const isSelectedElementLocked =
        selectedElements.find((el) => el.id === selectedElementId)?.locked ??
        false;
    const areAllSelectedElementsLocked =
        selectedElements.length > 0 &&
        selectedElements.every((el) => el.locked);
    const areSomeSelectedElementsLocked = selectedElements.some(
        (el) => el.locked
    );
    ...
    const handleElementPointUpdate = (
        elementId: string,
        pointIndex: number,
    ...
    const handleDragMultipleElements = (dx: number, dy: number) => {
        ...
        if (
            ...
            currentSelectedElements.some((el) => el.locked)
        ) {
            return;
        }
